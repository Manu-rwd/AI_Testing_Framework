// Fill M8 review columns for Accesare (Adaugare) and append rows to reach >=10
// Preserves UTF-8 BOM and CRLF; RFC4180-safe quoting
...
function detectHasBOM(buf) {
  return buf.length > 0 && buf[0] === 0xef && buf[1] === 0xbb && buf[2] === 0xbf;
}
...
function main() {
  ...
  if (idxModule < 0 || idxTF < 0 || idxDisp < 0 || idxNeeds < 0 || idxNotes < 0 || idxReviewer < 0 || idxReviewedAt < 0) {
    throw new Error("Required review columns missing. Run plan:review:init first.");
  }

  // Update existing Adaugare rows
  ...
  // Append synthetic Adaugare rows to reach at least 10 reviewed
  ...
  for (let i = 0; i < toAdd + 2; i++) { // add a couple extra to be safe
